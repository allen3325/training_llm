...
def transform_data_format(data):
    # transform data
    SUGGEST_SYS_PROMPT = f"You are a helpful AI assistant built by MediaTek Research. The user you are helping speaks Traditional Chinese and comes from Taiwan.\n{data['instruction']}"
    query1 = f"{data['input']}"
    data["formated"] = (
        f"{SUGGEST_SYS_PROMPT} [INST] {query1} [/INST] {data['output']}</s>"
    )
    return data
...
dataset = load_from_disk("/user_data/data/LogiCoT_zh_tw")  # load dataset
...
dataset = dataset.map(
    transform_data_format, remove_columns=["input", "instruction", "output"]
)  # transform data to llm format

model_path = "/user_data/project/Breeze/Breeze-7B-Instruct-v1_0"  # base model

new_model = "Breeze-v1_0-CoT-TEST"  # new model

output_dir = "./results"  # tensorboard 結果
